...
def _fold_bn(conv_module, bn_module, avg=False):
    ...
    if avg:
        y_mean = allaverage(y_mean)
        y_var = allaverage(y_var)
...
def search_fold_bn_ema_training(model, use_ema=False, **kwargs):
    model.eval()
    prev = None
    prev_name = 'conv'
    for n, m in model.named_children():
        if is_bn(m) and is_absorbing(prev):
            if use_ema:
                setattr(model, prev_name, EMAConv2dBN(prev, m, **kwargs))
            else:
                setattr(model, prev_name, PeriodicLookAheadConv2dBN(prev, m, **kwargs))
            setattr(model, n, StraightThrough())
        else:
            search_fold_bn_ema_training(m, use_ema, **kwargs)
        prev = m
        prev_name = n
